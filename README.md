...
本项目提供一套**AI 时代面向业务结果的评测方法与示例底座**，帮助团队将真实业务需求转化为可重复执行、可验证结果、可分析失败原因的评测任务。

被测对象既可以是直接完成工作的 AI Agent，也可以是通过 HTTP 接入的应用、算法或工作流。
...
1. **验证业务方案是否真的可用**
   ...
   不只看回答是否合理、文件是否生成或接口是否返回成功，还要检查结果是否完整、计算是否正确、异常是否被正确处理，为上线验收提供依据。
...
对于 Agent，分项评分、交付物和运行轨迹还可用于模型选型与过程定位。一次评测中可复用的经验不只是 trajectory，而是任务与输入版本、运行条件、交付物、分项结果、过程轨迹以及成本和耗时等完整运行证据。

当业务流程逐步成熟后，经授权、筛选的运行证据可以支持下游优化：成功轨迹、失败修正样本和偏好对可用于蒸馏与后训练；足够多且口径一致的数据可用于训练任务路由或失败预测模型；稳定且边界清楚的成功模式可进一步固化为规则、工作流或普通程序，把 Agent 保留给开放和长尾任务。这些是评测数据的后续用途，不是本仓库内置的训练或流程生成能力。无论最终实现是 Agent、模型、工作流还是普通程序，都应继续使用一致的业务验收标准和贴近目标分布的独立评测集，验证效果、版本回归与适用边界。

![业务评测驱动的实现选择工作流](images/workflow.png)
...
<p align="center">
  <img src="images/1.png" alt="Harbor Results Explorer 的运行概览：筛选器、任务及模型稳定性概览" width="100%" />
</p>
<p align="center">
  <img src="images/2.png" alt="Harbor Results Explorer 的 Trial 详情：分项评分与交付结果" width="100%" />
</p>
